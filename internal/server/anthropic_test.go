...
func TestAnthropicRouteRewritesModelAndUsesRouteAuth(t *testing.T) {
	var gotAuth string
	var gotBody []byte
	var gotPath string
	defaultHits := 0
	...
	if defaultHits != 0 {
		t.Fatalf("default upstream was hit %d time(s)", defaultHits)
	}
	if gotAuth != "Bearer route-token" {
		t.Fatalf("route auth = %q", gotAuth)
	}
	if gotPath != "/v1/messages?beta=true" {
		t.Fatalf("path = %q", gotPath)
	}
	if !bytes.Contains(gotBody, []byte(`"gpt-6-luna"`)) || bytes.Contains(gotBody, []byte(`"claude-haiku-4-5"`)) {
