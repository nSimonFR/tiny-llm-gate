...
// fallback_on_status must reach codex hops too: a usage-limit 429 overflows
// to the next model, while a 5xx outside the explicit list does not.
func TestCodexProvider_FallbackOnStatus(t *testing.T) {
	for _, tc := range []struct {
		...
		badCodex := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			_, _ = io.WriteString(w, `{"error":{"message":"limit"}}`)
		}))
		fallback := newMockUpstream(200, `{"id":"x","choices":[{"message":{"content":"from-fallback"}}]}`)
		...
		s := buildServer(t,
			...
			map[string]config.Model{
				"gpt-5.5": {Provider: "codex", UpstreamModel: "gpt-5.5", Fallback: []string{"gemma"}, FallbackOnStatus: []int{429}},
				"gemma":   {Provider: "ollama", UpstreamModel: "gemma4:e4b"},
			},
			nil,
		)
		...
		gotFallback := strings.Contains(rec.Body.String(), "from-fallback")
		if gotFallback != tc.wantFallback {
			t.Errorf("status %d: fallback = %v, want %v (code=%d body=%s)", tc.status, gotFallback, tc.wantFallback, rec.Code, rec.Body.String())
		}
		if !tc.wantFallback && rec.Code != tc.status {
			t.Errorf("status %d: client got %d, want the upstream status passed through", tc.status, rec.Code)
		}
		badCodex.Close()
		fallback.Close()
	}
